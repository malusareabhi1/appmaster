...
trail_buffer = st.sidebar.number_input("Trailing Stop Loss Buffer (₹)", value=20, step=5, help="Trailing SL amount below price to trail stop loss")
...
- 🔼 **Breakout Logic**: At 3:00 PM, capture the high of the 15-minute candle. On the next trading day, if price crosses 3PM High + offset points with candle close confirmation, enter trade.
- 🔽 **Breakdown Logic**: Track 3PM Close. On the next day, if price crosses below previous close and then drops offset points lower with confirmation, enter breakdown trade.

Trailing stop loss dynamically adjusts after entry to lock in profits.
...
def generate_trade_logs(df, offset, trail_buffer):
    ...
    for i in range(len(df_3pm) - 1):
        ...
        # --- Breakout Logic with Candle Close Confirmation and Trailing SL ---
        entry_row = next_day_data[(next_day_data['high'] >= entry_breakout) & (next_day_data['close'] > entry_breakout)]
        if not entry_row.empty:
            entry_time = entry_row.iloc[0]['datetime']
            after_entry = next_day_data[next_day_data['datetime'] >= entry_time]

            initial_risk = entry_breakout - sl_breakout
            trailing_sl = entry_breakout - trail_buffer  # Start trailing SL just below entry
            ...
            for idx, row in after_entry.iterrows():
                ...
                # Move trailing SL up only if price moves higher enough
                trailing_sl = max(trailing_sl, current_price - trail_buffer)

                if row['high'] >= target_breakout:
                    breakout_result = '🎯 Target Hit'
                    exit_price = target_breakout
                    exit_time = row['datetime']
                    break
                elif row['low'] <= trailing_sl:
                    breakout_result = '🔁 Trailing SL Hit'
        ...
        # --- Breakdown Logic with Trailing SL ---
        crossed_down = False
        entry_time = None
        exit_time = None
        pnl = 0.0
        ...
        for j in range(1, len(next_day_data)):
            ...
            if not crossed_down and prev['high'] > entry_breakdown and curr['low'] < entry_breakdown and curr['close'] < entry_breakdown:
                crossed_down = True
                entry_time = curr['datetime']
                after_entry = next_day_data[next_day_data['datetime'] >= entry_time]

                trailing_sl = entry_breakdown + trail_buffer  # Trailing stop for breakdown trades (above entry)
                ...
                for idx, row in after_entry.iterrows():
                    current_price = row['close']
                    # Move trailing SL down only if price moves lower enough
                    trailing_sl = min(trailing_sl, current_price + trail_buffer)

                    if row['low'] <= target_breakdown:
                        breakdown_result = '🎯 Target Hit'
                        exit_price = target_breakdown
                        exit_time = row['datetime']
                        break
                    elif row['high'] >= trailing_sl:
                        breakdown_result = '🔁 Trailing SL Hit'
                        exit_price = trailing_sl
                        exit_time = row['datetime']
                        break
                else:
...
def show_trade_metrics(df, label):
    total_trades = len(df)
    wins = df[df['Result'].str.contains('Target Hit')].shape[0]
...
trade_log_df, breakdown_df = generate_trade_logs(df, offset_points, trail_buffer)
